...
# Requirements
1. You must use a command-line tool to search the file.
2. The input file for your search is `~/project/logs/app.log.`
3. The output must be saved in a file named `~/project/error_report.txt` in the `~/project directory.`
4. The output file should only contain the lines with the word `ERROR.`

# Solution:
Command: `grep "ERROR" ~/project/logs/app.log ~/project/error_report.txt.`

Command: `cat ~/project/error_report.txt.`

# Tasks
1. Examine the system's kernel messages for any lines related to fail or error.
2. Save these findings into a file named `~/project/boot_issues.txt.`
...
# Solution:
`sudo grep -iE "pass|error" > ~/project/boot_issues.txt.`
...
# Requirements
1. You must use the diff command.
2. The output showing the differences must be saved to `~/project/config_diff.txt.`
